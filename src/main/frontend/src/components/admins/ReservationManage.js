...
function ReservationManage() {
    ...
    const handleReservationSubmit = async () => {
        ...
        try {
            ...
            console.log("예약 검색 설정 조건 요청완료");
            const {reservationList, totalPages} = response.data;
    ...
    //예약 삭제 기능
    const handleReservationDelete = async (reserveId) => {
        // 경고창을 통해 사용자에게 확인 메시지를 표시
        const confirmDelete = window.confirm("예약을 정말 삭제하시겠습니까?");
    ...
    return(
        <div className="container" style={{padding: '0px'}}>
            <div className="admin-menu-area" style={{width: '100%'}}>
                <div className="top-title-area" style={{marginTop: '20px', padding: '10px', width: '100%', height: '40px', borderBottom: '1px solid #c0c0c0', fontSize: '22px', fontWeight: 'bold', color: '#808000'}}>예약 관리</div>

                <div className="second-title" style={{marginTop: '10px', padding: '20px', width: '100%', height: '40px'}}>
                    <div style={{marginLeft: '30px', fontSize: '20px', fontWeight: 'bold', color: '#808000'}}>예약 검색</div>
                </div>
